...
def connect_device(address: str) -> str:
    ...
    try:
        result = subprocess.run(
            ["adb", "connect", address], capture_output=True, text=True, check=False
        )
...
def disconnect_device(address: str) -> str:
    try:
        result = subprocess.run(
            ["adb", "disconnect", address], capture_output=True, text=True, check=False
        )
        return result.stdout.strip()
    except Exception as e:
...
def pair_device(address: str, code: str) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ["adb", "pair", address, code], capture_output=True, text=True, check=False
        )
